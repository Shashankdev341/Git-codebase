...
import {
  WebGLErrorBoundary,
  WebGLFallback,
} from "@/components/ui/webgl-liquid-utils/webgl-error-boundary";
...
const FALLBACK_DEEP = "#13061b";
const FALLBACK_MID = "#210b2c";
const FALLBACK_HIGHLIGHT = "#ffd166";
...
export function WebGLLiquid({
  ...
  const fallbackContent = (
    <div
      className={cn(
        "relative flex min-h-screen w-full items-center overflow-hidden bg-[#13061b] text-white",
        className,
      )}
  ...
  return (
    <WebGLErrorBoundary fallback={fallbackContent}>
      <div
        ref={hostRef}
        className={cn(
          "fixed inset-0 w-screen h-screen overflow-hidden text-[#ffd166] pointer-events-none z-0",
          className,
        )}
        style={{ containerType: "size", ...style }}
        {...props}
      >
        ...
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-r from-[#13061b]/80 via-[#210b2c]/40 to-transparent" />
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_65%_40%,rgba(188,150,230,0.18),transparent_45%)]" />
